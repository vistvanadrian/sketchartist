...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (!process.env.API_KEY) {
      console.warn("Sketch Artist AI: API_KEY environment variable is missing!");
    }
  }, []);
  ...
  const handleCreateSketch = async () => {
    ...
    if (!process.env.API_KEY) {
        setState(prev => ({ ...prev, error: "Hiba: Az API kulcs nincs beállítva a Vercel környezeti változói között (API_KEY)." }));
        return;
    }
    ...
    const getButtonStyle = (style: SketchStyle) => {
        ...
        if (style === 'ghibli') {
            return isSelected 
                ...
                : 'bg-indigo-50 dark:bg-indigo-900/20 border-indigo-200 dark:border-indigo-800 text-indigo-700 dark:text-indigo-300 hover:border-indigo-400';
        }
        ...
        <div className="grid grid-cols-4 gap-2">
            {SKETCH_STYLES.map(style => (
                <button
                    ...
                    className={`px-3 py-2 text-[10px] sm:text-xs font-bold rounded-lg transition-all duration-200 border-2 ${getButtonStyle(style)} disabled:opacity-50 disabled:cursor-not-allowed`}
                >
        ...
        <div className="space-y-6">
            {[
                ...
                <div key={control.key} className={`${control.disabled ? 'opacity-50' : ''}`}>
                    ...
                    />
                    {control.disabled && <p className="text-[10px] text-slate-400 mt-1 italic">Ez a stílus fix értékeket használ ehhez a paraméterhez.</p>}
                </div>
            ))}
        </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors font-sans">
      <header className="p-4 sm:p-6 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 sticky top-0 z-20 shadow-sm backdrop-blur-md bg-opacity-90 dark:bg-opacity-90">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3 cursor-pointer group" onClick={resetApp}>
            <div className="p-2 bg-amber-500 rounded-lg text-white shadow-lg shadow-amber-500/30 group-hover:scale-105 transition-transform"><i className="fas fa-pen-nib text-xl sm:text-2xl"></i></div>
            <h1 className="text-xl sm:text-2xl font-black tracking-tight uppercase">Sketch Artist <span className="text-amber-500">AI</span></h1>
          </div>
          <button onClick={() => setState(prev => ({ ...prev, theme: prev.theme === 'LIGHT' ? 'DARK' : 'LIGHT' }))} className="p-3 w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded-full bg-slate-100 dark:bg-slate-800 text-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">{state.theme === 'LIGHT' ? '🌙' : '☀️'}</button>
        </div>
      </header>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
                  {(state.batchResultImages.length > 0 ? state.batchResultImages : state.batchSourceImages.map(s => ({ source: s, result: '', error: undefined }))).map((item, index) => (
                    <div key={index} className="relative group bg-slate-100 dark:bg-slate-800/50 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-800 shadow-sm animate-fade-in">
                      {item.error ? (<div className="aspect-square flex flex-col items-center justify-center text-center p-2 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400"><i className="fas fa-exclamation-triangle text-2xl mb-2"></i><p className="text-xs font-bold">Hiba</p><p className="text-[10px] opacity-80">{item.error}</p></div>) : item.result ? (<img src={item.result} className="w-full h-full object-cover aspect-square" alt="Result" />) : (<img src={item.source} className="w-full h-full object-cover aspect-square" alt="Source" />)}
        ...
        {showUpload && (
          <div className="max-w-4xl mx-auto bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 shadow-xl min-h-[400px] flex flex-col justify-center animate-fade-in border border-slate-100 dark:border-slate-800">
             <div className="space-y-8">
              ...
              {state.uploadMode === 'upload' ? (<div className="text-center space-y-6 animate-fade-in"><div className="border-4 border-dashed border-slate-200 dark:border-slate-700 rounded-3xl p-8 sm:p-12 hover:border-amber-400 dark:hover:border-amber-500 hover:bg-amber-50 dark:hover:bg-slate-800/50 transition-all cursor-pointer group relative overflow-hidden"><input type="file" accept="image/*" onChange={handleImageUpload} className="absolute inset-0 opacity-0 cursor-pointer z-10" multiple /><div className="text-5xl sm:text-6xl mb-6 text-slate-300 dark:text-slate-600 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300">🖼️</div><h2 className="text-xl font-bold mb-2">Húzd ide a fotóidat</h2><p className="text-sm text-slate-500 dark:text-slate-400">Válassz ki egy vagy több fájlt.</p></div></div>) : (<div className="text-center space-y-6 pt-2 animate-fade-in"><div><h2 className="text-2xl font-bold mb-2">Mit rajzoljak neked?</h2><p className="text-slate-500 dark:text-slate-400">Írd le angolul vagy magyarul, és generálok egy fotót vázlatnak.</p></div><div className="relative"><textarea value={state.generationPrompt} onChange={(e) => setState(prev => ({...prev, generationPrompt: e.target.value}))} placeholder="Pl: 'A futuristic city at sunset'..." className="w-full h-32 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 outline-none focus:ring-2 ring-amber-500 transition-all resize-none shadow-inner text-lg" /><div className="absolute bottom-4 right-4 text-slate-400 text-xs font-bold pointer-events-none">AI</div></div><button onClick={handleGenerateFromPrompt} disabled={!state.generationPrompt} className="w-full py-4 bg-gradient-to-r from-slate-900 to-slate-800 dark:from-amber-500 dark:to-amber-600 text-white rounded-2xl font-black text-lg shadow-xl hover:shadow-2xl hover:scale-[1.01] active:scale-[0.99] transition-all disabled:opacity-50 flex items-center justify-center gap-3"><i className="fas fa-magic"></i> KÉP GENERÁLÁSA</button></div>)}
              {state.error && !state.isProcessing && <div className="text-red-600 dark:text-red-400 mt-4 text-center p-4 bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-200 dark:border-red-900/50 flex items-center justify-center gap-2 font-medium"><i className="fas fa-exclamation-circle"></i> {state.error}</div>}
              {state.isProcessing && !state.sourceImage && <div className="text-slate-500 mt-4 text-center p-4 flex items-center justify-center gap-2 animate-pulse"><div className="w-5 h-5 border-2 border-amber-500 border-t-transparent rounded-full animate-spin"></div> Művelet folyamatban...</div>}
            </div>
          </div>
        )}
        ...
        {showMainView && (
           <div className="max-w-5xl mx-auto space-y-6 animate-fade-in">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-4 sm:p-8 shadow-2xl border border-slate-100 dark:border-slate-800">
             {(state.isProcessing && !state.resultImage) ? (<div className="py-24 sm:py-32 text-center"><div className="relative w-24 h-24 mx-auto mb-8"><div className="absolute inset-0 border-4 border-slate-200 dark:border-slate-700 rounded-full"></div><div className="absolute inset-0 border-4 border-amber-500 border-t-transparent rounded-full animate-spin"></div><i className="fas fa-magic absolute inset-0 flex items-center justify-center text-3xl text-amber-500 animate-bounce"></i></div><h3 className="text-2xl font-bold text-slate-800 dark:text-slate-100">{TRANSLATIONS.HU.artistAtWork}</h3><p className="text-slate-500 dark:text-slate-400 mt-3">A mesterséges intelligencia épp a vonalakat húzza...</p></div>) : state.error ? (<div className="text-center py-20"><div className="text-red-500 text-5xl mb-6"><i className="fas fa-exclamation-triangle"></i></div><h3 className="text-2xl font-bold text-red-600 dark:text-red-400 mb-2">Hiba történt</h3><p className="text-slate-500 dark:text-slate-400 mt-2 mb-8 max-w-md mx-auto">{state.error}</p><button onClick={resetApp} className="px-8 py-3 bg-amber-500 text-white rounded-xl font-bold hover:bg-amber-600 transition-colors shadow-lg shadow-amber-500/30">Vissza a főoldalra</button></div>) : (
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  <div className="lg:col-span-7 space-y-6">
                    <div className={`relative bg-slate-100 dark:bg-slate-950 p-2 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-inner overflow-hidden group ${state.isProcessing ? 'opacity-90 pointer-events-none' : ''}`}>
                       ...
                       {state.isProcessing && state.resultImage && (<div className="absolute inset-0 flex flex-col items-center justify-center bg-white/50 dark:bg-black/50 backdrop-blur-sm z-10 transition-opacity"><div className="w-12 h-12 border-4 border-amber-500 border-t-transparent rounded-full animate-spin mb-3"></div><span className="font-bold text-slate-800 dark:text-white drop-shadow-md">Újragenerálás...</span></div>)}
                    </div>
                     {state.history.length > 1 && (<div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800"><label className="block text-[10px] font-bold mb-3 uppercase text-slate-400 tracking-wider">Verzió Előzmények</label><div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">{state.history.map((version, index) => (<button key={index} onClick={() => handleVersionSelect(version)} className={`relative flex-shrink-0 w-14 h-14 sm:w-16 sm:h-16 rounded-lg overflow-hidden transition-all border-2 ${version === state.resultImage ? 'border-amber-500 scale-105 shadow-md ring-2 ring-amber-500/20' : 'border-transparent opacity-60 hover:opacity-100'}`}><img src={version} alt={`v${index}`} className="w-full h-full object-cover" /></button>))}</div></div>)}
                  </div>
                  <div className="lg:col-span-5 space-y-6">
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-2xl border border-slate-200 dark:border-slate-700">
                        ...
                        <div className="space-y-8 mb-8">
                          ...
                          :
                            (<button onClick={handleCreateSketch} disabled={state.isProcessing} className="w-full py-4 bg-gradient-to-r from-amber-500 to-amber-600 text-white rounded-2xl font-black text-lg shadow-xl hover:shadow-2xl hover:scale-[1.01] active:scale-[0.99] transition-all disabled:opacity-50 flex items-center justify-center gap-3"><i className="fas fa-pencil-ruler"></i> {TRANSLATIONS.HU.createSketch}</button>)
                          }
                        </div>
                        {state.resultImage && (
                          <>
                      ...
                      <div className="flex flex-col gap-3">
                           <div className="flex gap-2">
                                <select 
                                    ...
                                    className="px-3 py-4 rounded-2xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 font-bold focus:outline-none focus:border-emerald-500 cursor-pointer text-sm"
                                >
                                    <option value="png">PNG</option>
                                    <option value="jpeg">JPG</option>
                                    <option value="svg">SVG</option>
                                </select>
                                <button onClick={() => handleDownload(state.resultImage!)} disabled={state.isProcessing} className="flex-1 py-4 bg-emerald-600 text-white rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-emerald-500 transition-colors shadow-lg shadow-emerald-600/20 disabled:opacity-50 active:scale-[0.98] transform text-base sm:text-lg"><i className="fas fa-download"></i> KÉP LETÖLTÉSE</button>
                           </div>
                           <button onClick={resetApp} disabled={state.isProcessing} className="w-full py-3 bg-transparent text-slate-500 dark:text-slate-400 font-bold flex items-center justify-center gap-2 hover:text-slate-700 dark:hover:text-slate-200 transition-colors text-xs sm:text-sm"><i className="fas fa-plus-circle"></i> Új alkotás kezdése</button>
                      </div>
                    )}
                  </div>
                </div>
